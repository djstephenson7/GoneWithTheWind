...
class LibrarySpec extends FunSuite {
  ...
  test("Find by full ISBN") {
    val mockBookList: List[Book] = List(Book("mockName1", "mockAuthor1", "ISBN1"), Book("mockName2", "mockAuthor2", "ISBN2"))
    val library = new Library
    library.search_isbn("ISBN1", mockBookList) shouldBe Some(Book("mockName1", "mockAuthor1", "ISBN1"))
    library.search_isbn("ISBN2", mockBookList) shouldBe Some(Book("mockName2", "mockAuthor2", "ISBN2"))
